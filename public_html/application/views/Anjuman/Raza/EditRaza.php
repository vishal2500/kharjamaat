...
<div id="fields" class="query-form">
    ...
    <div class="submit">
        ...
        <button type="submit" class="btn btn-primary w100percent-xs mbm-xs" onclick="Confirm();">Confirm</button>
    </div>
</div>
<script>
    let raza = <?php $raza_json = json_encode($raza);
    echo $raza_json; ?>;
    ...
    function handlviewoptions(index) {
        ...
        raza.fields.fields[Number(index)].options.forEach(element => {
            tabledata += `<tr class="form-group"><td><input type="text" class="form-control" name="${element.id}" value="${element.name}"></td></tr>`
        });
        table.innerHTML = tabledata;
        let t = document.getElementById('options-table')
        t.innerHTML = "";
        t.appendChild(table)
    }
    function cancelview() {
    ...
    function Confirm() {
        let raza_id_input = document.getElementById('raza-id');
        raza_id_input.value = raza.id;
        let raza_option_input = document.getElementById('option-id');
        raza_option_input.value = current_option_index;
    ...
    $(document).ready(function () {
        $('#options-form').submit(function (event) {
            ...
            $.ajax({
                type: 'POST',
                url: "<?php echo base_url('admin/modifyrazaoption'); ?>",
                data: formData,
    ...
    function Addoption() {
        let optiontable = document.getElementById('opt-table')
        let tr = document.createElement('tr');
        tr.className = "form-group";
        var elementCount = optiontable.getElementsByTagName('tr').length;
        console.log(elementCount)
        tr.innerHTML = `<td><input type="text" class="form-control" name="${elementCount}"></td>`
        optiontable.appendChild(tr);
    }
    function removeOption() {
        let optiontable = document.getElementById('opt-table')
        var lastElement = optiontable.lastChild;
